...
function Get-DeclaredDocId([string]$Text, [string]$Path) {
    $match = [regex]::Match($Text, '(?m)^document_id:\s*([^\s]+)\s*$')
    if ($match.Success) {
        return $match.Groups[1].Value.Trim("'").Trim('"')
    }
    return $null
...
$expectedCanonicalArtifacts = @{
    '.agents/research-governance.md' = 'research-governance.md'
...
foreach ($artifact in $manifest.canonical_artifacts) {
    ...
    $text = Read-Text $path
    $actual = Get-DeclaredVersion $text $path
    ...
    if ($artifact.document_id) {
        $actualDocId = Get-DeclaredDocId $text $path
        if ($actualDocId -and $actualDocId -ne [string]$artifact.document_id) {
            Add-Error "Manifest document_id for '$path' is '$($artifact.document_id)', but the artifact declares '$actualDocId'."
        }
    }
}
...
# Scientific safeguards verification
$researchGovRel = 'research-governance.md'
if (Test-Path -LiteralPath (Resolve-PackagePath $researchGovRel) -PathType Leaf) {
    $govText = Read-Text $researchGovRel
    $safeguardKeywords = @(
        'exploratory',
        'confirmatory',
        'feasibility',
    ...
    foreach ($kw in $safeguardKeywords) {
        if (-not ($govText -match "(?i)$kw")) {
            Add-Error "Scientific governance protocol '$researchGovRel' is missing required safeguard concept '$kw'."
        }
    }
}
...
Write-Output 'Scientific package consistency check passed.'
exit 0
